...
# this will leave all instances of Audio-only coded as 0,
# while all other instances will get coded as 1
# (try same with factors?)
...
Build a reduced model that doesn't contain the fixed effect of modality, but is otherwise identical to the full model (including the random effects structure and control parameter)
```{r}
...
ggplot(figuredata, aes(x = xvar, y = yvar)) + 
  stat_smooth(method = lm, se = FALSE, linetype = "solid", 
              formula = 'y ~ x', color = "black", size = .6) +
